...
Note: tested for heteroskedasticity on fitted model m01:

```{r eval=FALSE, include=FALSE}
#residuals vs. fitted plot for diag
...
Omitted NAs and used robust HC3 standard errors.
...
However, there is no evidence we can reject the null -- the P is 0.45 so over 0.05, and the confidence interval crosses 0 (95% CI includes 0, p = 0.45).
...
# pieces of robust models as named vectors
...
# sum of weights (respondents)
sum_w <- sum(model.weights(model.frame(m01)))

#blank space function for footer
...
#render
tbl_ttl <- "Table 1. Slavery's Impact on Contemporary Racial Resentment, Using Soil Suitability for Cotton as an Instrument"
...
```{r echo=FALSE}
#for endogenous variable in the model, regress on all exogenous ind variables and all instruments. generate predicted values from those regressions.  
...
Does two-stage approach get the same as ivreg 2SLS? (yes)
