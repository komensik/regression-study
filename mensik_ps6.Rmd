...
a.  Created two new variables in `contrib` that are the natural log of `faminc` and `given`, named  `lfaminc` and `lgiven`.
...
b. The CME of age on contributions reaches zero at 97.93.
...
b.  Plots of the conditional marginal effect of `age` on `given` across values of `age`:
...
b.  Plot of the predicted values of `given` across values of `age`:
...
b.  Work to find the marginal effects of family income on campaign contributions for individuals who are 30, 50, and 70 years old, using the coefficients from the regression table:
...
b.  Plot that shows the marginal effect of family income at each value of age. 
