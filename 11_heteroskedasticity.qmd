...
-   Education has the same effect regardless of year

-   **Year-to-year differences are shoved into the error term**

But vocabulary scores *systematically change by year*:
...
So the error term is **correlated within year,** and violates :$\text{Cov}(u_i, u_j) \neq 0$

if year affects both vocab (DV) and ed (X) – OVB
...
Adding fixed effects (m2_a) does 3 things:

1.  creates year dummies, giving each year its own intercept

    1.  one year excluded as a baseline

2.  Education is identified within year

    1.  The coefficient on `education` is now estimated by comparing **people with different education levels *within the same year***.

    2.  – gets rid of between year variation

3.  removes omitted group-level confounding (test difficulty or measurement changes, cohort effects) – "uses only within-group variation"
